...
def resample_tif(input,reference,output):
	...
	dst = DriverGTiff.Create(dst_filename,wide,high,1,gdal.GDT_Float32,options=['COMPRESS=LZW','PREDICTOR=2','BIGTIFF=YES'])
...
#	rgbimage=list(im.getdata())#convert('RGB')
	data = np.asarray(im)
	#data = nlr.unstructured_to_structured(data).astype('O')
	data = np.where(np.isnan(data),0.0,data);print(data.shape)
...
def resample_sleuth_growth_tif(input,reference,output):
	color_dict={(255,0,0):1, #Seed urban
	    (0,255,0):2, #Diffusion
            (0,0,255):6, #Not used
            (255,255,0):3, #Breed
            (255,255,255):4, #Spread
            (0,255,255):5,
            (0,0,0):0} #Road-influenced
	...
	DriverGTiff = gdal.GetDriverByName('GTiff')
	dst = DriverGTiff.Create(dst_filename,wide,high,1,gdal.GDT_Int16,options=['COMPRESS=LZW','PREDICTOR=2','BIGTIFF=YES'])
	dst.SetGeoTransform(pop_geotrans)
	dst.SetProjection(pop_prj)
	outband = dst.GetRasterBand(1)
	outband.WriteArray(data)
