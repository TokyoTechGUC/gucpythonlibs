...
Functions:
    ...
    def __init__(self,files,outdir='./',nctype=None,overwrite=False):
        def check_type(self):
            ...
            if self.nctype==None:
                print("No type specified. Determining type from either WRF, CESM (and sub-models), or CDO outputs.")
                ...
                for key,value in ftypes.items():
                    try:
                        ...
                        if len(df.variables['XLAT'].shape)==2:
                            self.nctype='cdowrf'
                        print(f"{self.filecount} {value.upper()} file(s) detected (nctype={self.nctype}).")
                        break
                    except:
                        pass
                if self.nctype==None:
    ...
    def draw_map(self,variable=None,figsize=(8,6),coastwidth=0.0,dpi=100,title='',adjust_hr=0,**kwargs):
        self.coastwidth=coastwidth
        self.variable=variable
        self.title=title
        self.fig,self.ax = plt.subplots(figsize=figsize,dpi=dpi,constrained_layout=False)
        self.ax.clear()
        def gen_time(time,source,startdate,adjust_hr):
            units = source.units
            if 'minutes since simulation start' in source.description:
                time = datetime.strptime(startdate,'%Y-%m-%d_%H:%M:00')+timedelta(minutes=time,hours=adjust_hr)
            else:
                time = datetime.strptime(units,'minutes since %Y-%m-%d %H:%M:00')+timedelta(minutes=time,hours=adjust_hr)
            return time
        def construct_shell_cdowrf(self,**kwargs):
            df = nc.Dataset(self.files[0],'r')
            lats = df.variables['XLAT'][:,0]
            lons = df.variables['XLONG'][0,:]
            ny = lats.shape[0]; nx = lons.shape[0]
            m = Basemap(llcrnrlon=np.min(lons),llcrnrlat=np.min(lats),urcrnrlon=np.max(lons),urcrnrlat=np.max(lats),resolution='h',ax=self.ax)
        ...
        def draw_cdowrf(self,adjust_hr,**kwargs):
            ...
            for count,ifil in enumerate(self.files):
                ...
                for it in range(df.dimensions['XTIME'].size):
                    ...
                    time = gen_time(int(time[it]),time,df.SIMULATION_START_DATE,adjust_hr)
                    if self.title=='':
                    ...
                    if os.path.exists(outfile)&(not self.overwrite):
                        print(f"Skipping {outfile} since it exists. To overwrite, set overwrite=True")
                        continue
                    if self.variable == 'tc2':
                        var = df.variables['T2']
                        ax1 = ax.pcolormesh(self.x,self.y,var[it,:,:]-273.15,shading='auto',**kwargs)
                    else:
                        var = df.variables[self.variable]
                        ax1 = ax.pcolormesh(self.x,self.y,var[it,:,:],shading='auto',**kwargs)
                    m.drawcoastlines(linewidth=self.coastwidth)
                    ax.set_title(f'{self.title} {time.strftime("%Y/%m/%d %H:%M")} (UTC+{adjust_hr})')
                    if (count == 0)&(it == 0):
                        cbar = fig.colorbar(ax1,ax=ax,orientation='horizontal')
                        cbar.ax.set_title(f'{var.description.title()} ({degree+"C" if self.variable=="tc2" else var.units})')
                    else:
        ...
        def draw_wrf(self,adjust_hr,**kwargs):
            ...
            for count,ifil in enumerate(self.files):
                ...
                for it in range(df.dimensions['Time'].size):
                    ...
                    time = gen_time(int(time),time,df.SIMULATION_START_DATE,adjust_hr)
                    if self.title=='':
                        self.title=self.variable
                    outfile = f'{self.outdir}/{self.title.replace(" ","_")}_{time.strftime("%Y%m%d%H%M")}_UTCplus_{adjust_hr:02d}.png'
                    if os.path.exists(outfile)&(not self.overwrite):
                    ...
                    if self.variable == 'tc2':
                        ...
                        ax1 = ax.pcolormesh(self.x,self.y,var-273.15,shading='auto',**kwargs)
                    else:
                        var = getvar(df,self.variable,timeidx=it)
                        ax1 = ax.pcolormesh(self.x,self.y,var,shading='auto',**kwargs)
                    m.drawcoastlines(linewidth=self.coastwidth)
                    ax.set_title(f'{self.title} {time.strftime("%Y/%m/%d %H:%M")} (UTC+{adjust_hr})')
                    if (count == 0)&(it == 0):
                        cbar = fig.colorbar(ax1,ax=ax,orientation='horizontal')
    ...
    def create_video(self,string,framerate=4,out='out.mp4',overwrite_output=True):
        ...
        (
        ffmpeg.input(string,pattern_type='glob',framerate=framerate).output(out,**output_options).run(overwrite_output=overwrite_output)
        )
