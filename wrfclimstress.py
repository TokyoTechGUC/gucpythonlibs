...
def Bernard(tas,dewp):
	...
	Tpwb = optimize.minimize_scalar(func_to_opt_Bernard,method='bounded',bounds=(np.min(dewp-1.0),np.max(tas+1.0))).x
	return Tpwb+273.15
...
def natglob_temp(Tair,dewp,RH,Pair,wind,MinWindSpeed,radiation,propDirect,cza,irad=1.0,SurfAlbedo=0.4):
	...
	Tnwb = optimize.minimize_scalar(func_to_opt_natglob,method='bounded',bounds=(np.min(dewp-1.0),np.max(Tair+1.0))).x
	return Tnwb
